...
int main()
{
    ...
    //...
    //parser_parseEmpleados(pFile,listaEmpleados);
    //fclose(pFile);

    //employee_mostrar(listaEmpleados);
    // Leer empleados de archivo data.csv
    if(parser_parseEmpleados(pFile,listaEmpleados)==0)
    {
        // Calcular sueldos
...
int generarArchivoSueldos(char* fileName,LinkedList* listaEmpleados)
{
    ...
    if(pFile != NULL)
    {
        ...
        for(i = 0; i < size; i++)
        {
            auxPunteroEmployee = ll_get(listaEmpleados,i);
            fprintf(pFile, "Sueldo: %d\n", auxPunteroEmployee->sueldo);
        }
    }
    fclose(pFile);
    return retorno;
